...
bool GenioBase::sendbuf() 
{
  ...
  if ( BufferLog ) 
    {
      //std::stringstream s;
      //s << "==>" << BytesWritten << " Bytes written.";
      //extfunc_logmessage( &(s.str()) );
      //extfunc_logmessage( &( BufferToHex( TXBuf, BytesWritten ) ) );
    }
  TXBufPos = 0;
  return ftStatus;
}
...
int GenioBase::readbuf( unsigned char *buffer, unsigned long int BytesToRead ) 
{
  ...
  //bool ftStatus = extfunc_readdata( buffer, BytesToRead, &BytesRead );
  bool ftStatus = ftdi->Read( buffer, BytesToRead, &BytesRead );
  if ( !ftStatus ) 
    {
    //if ( BufferLog )
	  //extfunc_logmessage( &(std::string("FT_Read failed" /*+ AnsiString( ftStatus )*/)) );
      return 0;
    }
  if ( BufferLog ) 
    {
            //std::stringstream s;
            //s << "<==" << BytesRead << " Bytes read.";
            //extfunc_logmessage( &(s.str()) );
            //extfunc_logmessage( &( BufferToHex( buffer, BytesRead ) ) );
